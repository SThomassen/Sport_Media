...
namespace MixedRealityToolkit.UX.AppBarControl
{
    public class AppBar : InteractionReceiver
    {
        ...
        /// </summary>
        [SerializeField] private float HoverOffsetYScale = 0.25f;
        public float OffsetY
        {
            get
            {
                return HoverOffsetYScale;
            }
            set
            {
                HoverOffsetYScale = value;
            }
        }
        ...
        /// </summary>
        [SerializeField] private float HoverOffsetZ = 0f;
        ...
        {
            if (boundingBox == null || boundingBox.Target == null || !boundingBox.Target.activeSelf)
            {
            ...
            AppBarStateEnum barState = AppBarStateEnum.Default;

            if (State != AppBarStateEnum.Hidden)
                barState = State;

            State = (!boundingBox.Target.GetComponent<Renderer>().enabled) ?
                    AppBarStateEnum.Hidden : barState;

            HoverOffsetZ = -Mathf.Abs(boundingBox.Target.transform.localScale.x);
            ...
            Vector3 lookat = (boundingBox.transform.position - finalPosition).normalized;
            if (lookat != Vector3.zero)
            {
                Vector3 eulerAngles = Quaternion.LookRotation(lookat, Vector3.up).eulerAngles;
                eulerAngles.x = 0f;
                eulerAngles.z = 0f;
